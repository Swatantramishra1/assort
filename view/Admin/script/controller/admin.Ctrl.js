app.controller('CommonCtrl', ['$scope', '$rootScope', '$localStorage', '$timeout', '$interval', '$filter', '$state', 'JobService', '$http', 'textAngularManager',
    function ($scope, $rootScope, $localStorage, $timeout, $interval, $filter, $state, JobService, $http, textAngularManager) {
        ...
        let stringData = "", stringData_blog = "";
        ...
        function readFile_blog() {
            ...
            if (this.files && this.files[0]) {
                ...
                FR.addEventListener("load", function (e) {
                    // document.getElementById("img").src       = e.target.result;
                    stringData_blog = e.target.result;
                    // console.log(stringData);
                    document.getElementById("blogImgSelected").src = stringData;
                });
        ...
        try {
            ...
            document.getElementById("img_blog").addEventListener("change", readFile_blog);
        } catch (e) {
        ...
        if ($state.current.name == 'updatenews') {
        ...
        if ($state.current.name == 'updateBlog') {
        ...
        $scope.newsupdate = function (dt) {
            $scope.news = dt;
            document.getElementById("newsImgSelected").src = dt.newsImage;
        }
        $scope.blogupdate = function (dt) {
            $scope.blog = dt;
            document.getElementById("blogImgSelected").src = dt.blogImage;
        }
        ...
        $scope.submitAddNewsData = function () {
            // alert(stringData.length)
            var date = new Date($scope.news.newsDate);
            var dateString = new Date(date.getTime() - (date.getTimezoneOffset() * 60000))
                .toISOString()
                .split("T")[0];
            $scope.news.newsDate = dateString;
            $scope.isLoader = true;
            if (stringData !== "") {
                $scope.news.newsImage = stringData;
            }
        ...
        $scope.submitAddblogData = function () {
            // alert(stringData.length)
            var date = new Date($scope.blog.blogPublishDate);
            var dateString = new Date(date.getTime() - (date.getTimezoneOffset() * 60000))
                .toISOString()
                .split("T")[0];
            $scope.blog.blogPublishDate = dateString;
            $scope.isLoader = true;
            if (localStorage.getItem("stringData_blog")) {
                $scope.blog.blogImage = localStorage.getItem("stringData_blog");
            }

            var postData = {
                "request": $scope.blog
            }
            var blogMedia = JobService.addblogAndMedia.PostPromise(postData);
            blogMedia.then(
                // OnSuccess function
                function (answer) {
                    $scope.isLoader = false;
                    stringData = "";
                    swal("Success", "blog posted successfully !", "success");
                    if (answer.data.GetLocationResult.ResponseCode == 0) {
                        $rootScope.GetLocationResultLists = answer.data.GetLocationResult.Result;
                    } else {
                        $scope.ErrorMessage = answer.data.GetLocationResult.ResponseMessage;
